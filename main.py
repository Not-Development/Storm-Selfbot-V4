...
class TokenJoiner:
    ...
    async def accept_invite(self, token, invite_code, proxy, status_message):
        global joined_count, failed_count
        payload = {"session_id": "".join(random.choices(string.ascii_lowercase + string.digits, k=16))}
        
        proxy_config = {"http": f"http://{proxy}", "https": f"https://{proxy}"} if proxy else None
        ...
        try:
            ...
            async with asyncio.Lock():
                if status == 200:
                    self.joined_count += 1
                    msg = f"✅ Token {token[:10]}... joined successfully!"
                else:
                    self.failed_count += 1
                    msg = f"❌ Token {token[:10]}... failed to join ({status})"
                print(msg)

                await status_message.edit(
                    content=f"```\nJoining Progress:\nSuccess: {self.joined_count}\nFailed: {self.failed_count}\nLast Action: {msg}```"
                )

        except Exception as e:
            async with asyncio.Lock():
                self.failed_count += 1
                error_msg = f"[ERROR] {token[:10]}... failed: {e}"
                print(error_msg)
                await status_message.edit(
                    content=f"```\nError with token {token[:10]}...: {str(e)}```"
                )

@bot.command()
async def tjoin(ctx, invite_link: str, token_count: int):
    joiner = TokenJoiner()

    invite_code = invite_link.split("/")[-1]
    tokens = joiner.tokens[:token_count]
    proxies = joiner.proxies
    ...
    proxy_iter = iter(proxies) if proxies else iter([None] * len(tokens))
    tasks = []
    ...
    await status_message.edit(
        content=f"```Join process completed:\nSuccessful joins: {joiner.joined_count}\nFailed joins: {joiner.failed_count}```"
    )
...
async def help(ctx, *, command_name=None):
    ...
    if command_name is None:
        ...
        help_message = "```xml\n[ Help Panel ]\n"
        help_message += "<> = required arguments, [] = optional arguments\n\n"
        help_message += "[ Storm Selfbot V4 ]\n\n"

        # Iterate over all cogs and their commands
        for cog_name, cog in bot.cogs.items():
            help_message += f'[{cog_name}]\n'
            filtered_commands = [cmd for cmd in cog.get_commands() if not cmd.hidden]
            for command in filtered_commands:
                help_message += f'{command.qualified_name} {command.signature}\n'
        
        # Handle commands without cogs
        non_cog_commands = [cmd for cmd in bot.commands if cmd.cog is None and not cmd.hidden]
        if non_cog_commands:
        ...
        if command:
            ...
            help_message += f'{command.qualified_name} {command.signature}\n\n'
            help_message += f'{command.help if command.help else "No help available."}\n'
            help_message += "```"
            await ctx.send(help_message)
        else:
